...
export const teamApi = {
  ...
  getAll: async (params?: { type?: "developer" | "exco"; page?: number; limit?: number }) => {
    ...
    const extractMembers = (r: any) => {
      ...
      if (Array.isArray(resultsArr) && resultsArr.length > 0 && resultsArr[0]?.members !== undefined) {
  ...
  getDevelopers: async () => {
    const resp: any = await apiRequest<any>("/v1/teams", { params: { type: "developer" } })
    const extractMembers = (r: any) => {
      const resultsArr = r?.results ?? []
      if (Array.isArray(resultsArr) && resultsArr.length > 0 && resultsArr[0]?.members !== undefined) {
        return resultsArr.flatMap((team: any) => team.members ?? [])
  ...
  getExcos: async () => {
    const resp: any = await apiRequest<any>("/v1/teams", { params: { type: "exco" } })
    const extractMembers = (r: any) => {
      const resultsArr = r?.results ?? []
      if (Array.isArray(resultsArr) && resultsArr.length > 0 && resultsArr[0]?.members !== undefined) {
        return resultsArr.flatMap((team: any) => team.members ?? [])
