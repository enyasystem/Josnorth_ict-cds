    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <div className="profile-card rounded-3xl bg-white p-6 shadow-md h-[360px] max-w-xs mx-auto flex flex-col justify-between">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 mt-4 text-center text-sm line-clamp-2 min-h-[3rem]">
            {shortBio}
          </p>
        </div>
        ...
        <div className="mt-4 flex justify-center">
          <Button
            className="px-6 py-2 rounded-full bg-green-600 text-white text-sm cursor-pointer"
        ...
        <div className="flex flex-col sm:flex-row gap-6 mt-2">
          {hasImage ? (
            <img
              src={img}
              alt={name}
              className="w-28 h-28 rounded-full object-cover border-4 border-green-100 mx-auto sm:mx-0"
            />
          ) : (
            <div className="w-28 h-28 rounded-full bg-green-50 border-4 border-green-100 flex items-center justify-center mx-auto sm:mx-0">
              <User className="w-14 h-14 text-green-600" />
            </div>
          )}
          <div className="flex-1">
            <p className="text-sm text-muted-foreground mb-2">{role}</p>
            <p className="text-sm text-gray-700 mb-4">{shortBio}</p>
            <div
              className="profile-socials flex items-center gap-3"
              aria-label={`${name} social links`}
            >
              {/* reuse same icons */}
              <a
