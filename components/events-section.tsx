    ...
    <section className="py-24 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading
            ? Array.from({ length: 6 }).map((_, idx) => (
                <Skeleton key={idx} className="h-40 rounded-lg bg-emerald-900/30" />
              ))
            : events.map((e, idx) => (
                <Reveal key={e.id} index={idx} className="animate-fade-in">
                  <article className="bg-emerald-900/30 border border-emerald-800 p-3 rounded-lg card-hover h-40 overflow-hidden flex flex-col">
                    ...
                    <div className="mt-2 flex-1">
                      ...
                      <p className="text-emerald-200 text-xs mt-1">{new Date(e.date).toLocaleDateString()}</p>
        ...
        <div className="text-center">
          <Button asChild variant="outline" size="lg" className="border-emerald-600 text-emerald-100 hover:bg-emerald-800/50 bg-transparent">
